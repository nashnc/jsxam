const calculateResult = () => {
  /**from html function will be calledd */
  ...
  let xecute =
    /* calling fuction  based on operation*/

    operation === "+"
      ? addTwo(numOne, numTwo) /**additon  */
      : operation === "-"
      ? cutThem(numOne, numTwo) /**substraction */
      : operation === "/"
      ? divideEm(numOne, numTwo) /**diviton */
      : bioLogy(numOne, numTwo); /**multiply */

  sendOutput(xecute);
};

const sendOutput = (xecute) => {
  document.getElementById(
    "result"
  ).textContent = `Result: ${xecute}`; /**sending to output */
};
